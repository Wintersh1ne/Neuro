...
layout_1 = [[g.Text('ПРИМЕР РАБОТЫ НЕЙРОНА', justification='center', expand_x=True, font='Impact 30 normal')],
            [g.Input('0', size=(13, 1), justification='center', key='Input_1', enable_events=True),
            ...
            [g.Input('1', enable_events=True, key='Input_2', size=(13, 1), justification='center'),
...
while True:
    ...
    if event == g.WIN_CLOSED:
        break
    if event == '-INPUT1-':
        if values['-INPUT1-'] != "" and (values['-INPUT1-'][-1] not in ('0123456789') or values['-INPUT1-'][0] not in ('0123456789')):
            g.popup("Only digits allowed")
            window['-INPUT1-'].update(values['-INPUT1-'][:-1])

    if event == '-INPUT2-':
        print(values['-INPUT2-'][1:])
        if values['-INPUT2-'] != "" and (values['-INPUT2-'][-1] not in ('0123456789') or values['-INPUT2-'][0] not in ('0123456789')):
            g.popup("Only digits allowed")
            window['-INPUT2-'].update(values['-INPUT2-'][:-1])
    if event == 'Input_1':
        if values['Input_1'] != "" and (values['Input_1'][-1] not in ('0123456789') or values['Input_1'][0] not in ('0123456789')):
            g.popup("Only digits allowed")
            window['Input_1'].update(values['Input_1'][:-1])
    if event == 'Input_2':
        if values['Input_2'] != "" and (values['Input_2'][-1] not in ('0123456789') or values['Input_2'][0] not in ('0123456789')):
            g.popup("Only digits allowed")
